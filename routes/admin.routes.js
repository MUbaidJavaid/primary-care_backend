...
router.get('/stats', protect, adminOnly, async (req, res, next) => {
  try {
    const [totalQueries, totalSessions, totalUsers, totalGuidelines, totalChunks, emergencyCount, urgentCount, routineCount] = await Promise.all([
    ...
    res.json({
      totalQueries, totalSessions, totalUsers, totalGuidelines, totalChunks,
      urgencyBreakdown: { Emergency: emergencyCount, Urgent: urgentCount, Routine: routineCount },
      recentLogs,
    });
  } catch (error) {
    next(error);
